//Youmax + PhotoMax - http://codecanyon.net/item/youmax-youtube-vimeo-video-grid-for-wordpress/10065614
...
(function ($) {
	...
	var getInstagramUserPhotos = function (userId,nextPageUrl,$photomaxContainer) {
		...
		if(null!=nextPageUrl && nextPageUrl!="") {
			...
			apiUserPhotosURL = "https://api.instagram.com/v1/users/"+userId+"/media/recent?count="+photomax_global_options.maxResultsToLoad+"&client_id="+photomax_global_options.instagramClientId;
		}
		...
		$.ajax({
			...
			success: function(response) {
				updateCache('instagram',response,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag);

				/*if(photomax_global_options.loadMode.indexOf("paginate")!=-1) {
					if(loadMoreFlag) {
						loadMoreFlag = false;
						paginateFlag = true;
					}
					...
					insertInstagramPhotos(response,null,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag);
				}*/
			},
	...
	//get photos of any user using instagram API
	getInstagramUserTaggedPhotos = function (userTagPair,nextPageUrl,$photomaxContainer) {
		//console.log('inside getUserPhotos');
		var loadMoreFlag = false, createFlag = true, paginateFlag = false;
		...
		$.ajax({
			...
			dataType: 'jsonp',
			success: function(response) { 
				updateCache('instagram',response,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag,hashTag);
				/*if(photomax_global_options.loadMode.indexOf("paginate")!=-1) {	
					if(loadMoreFlag) {
						loadMoreFlag = false;
						paginateFlag = true;
					...
					insertInstagramPhotos(response,hashTag,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag);
				}*/
			},
			error: function(html) { alert(html); },
			beforeSend: setHeader
	...
	getInstagramGlobalTaggedPhotos = function (hashTag,nextPageUrl,$photomaxContainer) {
		//console.log('inside getUserPhotos');
		var loadMoreFlag = false, createFlag = true, paginateFlag = false;
		...
		if(null!=nextPageUrl && nextPageUrl!="") {
			...
			apiUserPhotosURL = "https://api.instagram.com/v1/tags/"+hashTag+"/media/recent?count="+photomax_global_options.maxResultsToLoad+"&client_id="+photomax_global_options.instagramClientId;
		}
	...
	getGoogleAlbumPhotos = function (tabId,startIndex,$photomaxContainer) {
		...
		s = tabId.indexOf("_");
		albumId = tabId.substring(0,s);
		userId = tabId.substring(s+1);
		...
		$.ajax({
			...
			dataType: 'jsonp',
			success: function(response) { 
				updateCache('google',response,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag);
				/*if(photomax_global_options.loadMode.indexOf("paginate")!=-1) {
					if(loadMoreFlag) {
						loadMoreFlag = false;
						paginateFlag = true;
					...
					insertGooglePhotos(response,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag);
				}*/
			},
			error: function(html) { alert(html); },
			beforeSend: setHeader
	...
	getGoogleUserPhotos = function (userId,startIndex,$photomaxContainer) {
		...
		apiAlbumPhotosURL = "https://picasaweb.google.com/data/feed/api/user/"+userId+"?kind=photo&start-index="+startIndex+"&max-results="+photomax_global_options.maxResultsToLoad+"&v=2&alt=json";
		...
		$.ajax({
			...
			success: function(response) {
				...
				//console.log('max results to load: '+photomax_global_options.maxResultsToLoad);
				updateCache('google',response,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag);
	...
	displayGooglePhotoStats = function(response,$photomaxContainer) {
		...
		if(photomax_global_options.overlayType=='stat-1' || photomax_global_options.overlayType=='stat-2') {
			if(photoViewCount_raw>0) {
				...
				//playOverlayString = '<div class="photomax-play-overlay"><div class="photomax-play-icon-holder photomax-overlay-stat"><i class="fa fa fa-dot-circle-o"></i><span class="photmax-overlay-stat-text">'+photoViewCount+'</span></div></div>';
			}
		}
	...
	getFacebookAlbumPhotos = function (albumId,nextPageApiUrl,$photomaxContainer) {
		...
		$.ajax({
			...
			dataType: 'jsonp',
			success: function(response) { 
				updateCache('facebook',response,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag);			
				/*if(photomax_global_options.loadMode.indexOf("paginate")!=-1) {
					if(loadMoreFlag) {
						loadMoreFlag = false;
						paginateFlag = true;
					...
					insertFacebookPhotos(response,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag);
				}*/
			},
	...
	getFacebookPagePhotos = function (pageId,nextPageApiUrl,$photomaxContainer) {
		//console.log('inside getPlaylistVideos');
		//var pageTokenUrl = "";
		var loadMoreFlag = false, createFlag = true, paginateFlag = false;
	...
	getInstagramUserDetails = function (userId,tabId,$photomaxContainer) {
		...
		apiUrl = "https://api.instagram.com/v1/users/"+userId+"?client_id="+photomax_global_options.instagramClientId+"&count=1";
	...
	initPhotomax = function($photomaxContainer) {
		...
		/*if(photomax_global_options.displayType != 'popup') {
			$photomaxContainer.append('<div id="photomax-encloser"></div>');
		}*/
	...
	paginationWrapper = function($photomaxContainer,handle) {
		...
		if(handle=="more") {
			handlePagination($photomaxContainer,handle,false,true,false);
		} else {
			...
			handlePagination($photomaxContainer,handle,false,false,true);
		}
	...
	handlePagination = function($photomaxContainer,handle,createFlag,loadMoreFlag,paginateFlag) {
		...
		tabId = $photomaxContainer.find(".photomax-tab-hover").attr("id");
	...
	loadMoreItems = function($photomaxContainer,$photomaxLoadMoreDiv) {
		...
		if(null!=nextPageToken && nextPageToken!="undefined" && nextPageToken!="") {
			...
			if(tabId.indexOf("instagram_user_photos_")!=-1) {
				...
				getInstagramUserPhotos(innerId,nextPageToken,$photomaxContainer);
			} else if(tabId.indexOf("instagram_user_tagged_photos_")!=-1) {
				innerId=tabId.substring(29);
				getInstagramUserTaggedPhotos(innerId,nextPageToken,$photomaxContainer);
			} else if(tabId.indexOf("instagram_global_tagged_photos_")!=-1) {
				innerId=tabId.substring(31);
				getInstagramGlobalTaggedPhotos(innerId,nextPageToken,$photomaxContainer);
			} else if(tabId.indexOf("google_album_photos_")!=-1) {
				innerId=tabId.substring(20);
				getGoogleAlbumPhotos(innerId,nextPageToken,$photomaxContainer);
			} else if(tabId.indexOf("google_user_photos_")!=-1) {
				innerId=tabId.substring(19);
				getGoogleUserPhotos(innerId,nextPageToken,$photomaxContainer);
			}  else if(tabId.indexOf("facebook_album_photos_")!=-1) {
				innerId=tabId.substring(22);
				getFacebookAlbumPhotos(innerId,nextPageToken,$photomaxContainer);
			} else if(tabId.indexOf("facebook_page_photos_")!=-1) {
				innerId=tabId.substring(21);
				getFacebookPagePhotos(innerId,nextPageToken,$photomaxContainer);
			}	
	...
	getInstagramUserIdForTabs = function (userId,tab_prefix,$photomaxContainer,isSelected,tab_suffix) {
		...
		apiUrl = "https://api.instagram.com/v1/users/search?q="+userId+"&client_id="+photomax_global_options.instagramClientId+"&count=100";
	...
	getGoogleUserIdForTabs = function (userId,tab_prefix,$photomaxContainer,isSelected) {
		...
		apiUrl = "https://www.googleapis.com/plus/v1/people/"+userId/*.replace(/:/g,'+')*/+"?key="+photomax_global_options.googleApiKey;
	...
	displayChannelHeader = function($photomaxContainer) {
		...
		//<div class="photomax-subscribe-clean"><a href="https://instagram.com/'+channelTitle+'" target="_blank"><i class="fa fa-instagram fa-lg"></i>&nbsp;&nbsp;FOLLOW</a></div>
		
		/*
		if(photomax_global_options.skin.indexOf("clean")!=-1) {
			$photomaxContainer.find('#photomax-header-wrapper').append('<div id="photomax-header-info"><div id="photomax-header-title">'+channelTitle+'</div><div class="photomax-subscribe-clean-wrapper"><div class="photomax-subscribe"><div class="g-ytsubscribe" data-channelid="'+channelId+'" data-layout="default" data-count="default"></div></div></div><div id="photomax-header-bio">'+channelDescription+'</div><div id="photomax-header-website"><a href="'+userWebsite+'" target="_blank">'+userWebsite+'</a></div><div id="photomax-header-counts"><span class="photomax-header-posts"><span class="photomax-count">'+channelVideos+'</span> '+photomax_translator_text.videos+'</span><span class="photomax-header-followers"><span class="photomax-count">'+channelSubscribers+'</span> '+photomax_translator_text.subscribers+'</span><span class="photomax-header-following"><span class="photomax-count">'+channelViews+'</span> '+photomax_translator_text.views+'</span></div></div>');
		
			$photomaxContainer.find('#photomax-select-box').prepend('<div id="photomax-search-holder"><input id="photomax-search-box" type="text" placeholder="'+photomax_translator_text.search+'"/><i class="fa fa-search photomax-search-icon"></i></div>');
			$photomaxContainer.find('#photomax-select-box').append('<i class="fa fa-caret-down"></i>');
		} else {
			
			//old search in header
			//$photomaxContainer.find('#photomax-header-wrapper').append('&nbsp;&nbsp;&nbsp;&nbsp;<div class="photomax-subscribe"><div class="g-ytsubscribe" data-channelid="'+channelId+'" data-layout="default" data-count="default"></div></div><div id="photomax-stat-holder"><div id="photomax-search-holder"><input id="photomax-search-box" type="text" placeholder=""/><i class="fa fa-search photomax-search-icon"></i></div></div>');
			
			//added search in header for backward compatibility
			$photomaxContainer.find('#photomax-header-wrapper').append('<div id="photomax-stat-holder"><div id="photomax-search-holder-header"><input id="photomax-search-box-header" type="text" placeholder="Search"/><i class="fa fa-search photomax-search-icon-header"></i></div></div>');

			
			$photomaxContainer.find('#photomax-header-wrapper').append('&nbsp;&nbsp;&nbsp;&nbsp;<div class="photomax-subscribe"><div class="g-ytsubscribe" data-channelid="'+channelId+'" data-layout="default" data-count="default"></div></div>');
			
			$photomaxContainer.find('#photomax-select-box').prepend('<div id="photomax-search-holder"><input id="photomax-search-box" type="text" placeholder="'+photomax_translator_text.search+'"/><i class="fa fa-search photomax-search-icon"></i></div>');
			$photomaxContainer.find('#photomax-select-box').append('<i class="fa fa-caret-down"></i>');
			
			$photomaxContainer.find('#photomax-select-box').wrap('<div class="photomax-select-box-wrapper">');
	...
	displayUserDetails = function(response) {
		...
		$('.'+authorNameAsClass).find('.photomax-from-img').css('background-image','url('+authorImage+')');
	...
	insertInstagramPhotos = function(response,hashTag,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag) {
		...
		if(photomax_global_options.loadMode.indexOf("paginate")==-1) {
			//if not pagination then update tokens..
		...
		if(null!=hashTag && hashTag!="") {
			if(photomaxReloadCount>=photomax_global_options.maxAutoReloads) {
				photomaxReloadCount = 1;
			} else {
				$photomaxLoadMoreDiv.click();
				photomaxReloadCount++;
			}
		}*/
		...
		for(var i=0; i<photoArray.length; i++) {
			...
			if(null!=hashTag && hashTag!=""){
				//console.log(hashTag.replace(/_+/g,''));
				if(photoDescription.toLowerCase().indexOf(hashTag.replace(/_+/g,' '))==-1 &&
					photoDescription.toLowerCase().indexOf(hashTag.replace(/_+/g,''))==-1) {
					continue;
				}
			}
			*/
		...
		/*if(photomax_global_options.loadMode.indexOf("paginate")!=-1 && (null==nextPageToken || nextPageToken.indexOf("photomax-generated")==-1)) {
			if(photoArray.length > 0) {
				cache = $photomaxContainer.data('cache');
				cacheIndex = $photomaxContainer.data('cacheindex');				

				cache = cache.concat(photoArray);
				cacheIndex = cache.length - photoArray.length - 1;
				//console.log(cache);
				//console.log("cacheIndex : "+cacheIndex);
	...
	updateCache = function(network,response,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag,filter) {
		...
		handlePagination($photomaxContainer,handle,createFlag,loadMoreFlag,paginateFlag);
		...
		*/
		...
		for(var i=0; i<photoArray.length; i++) {
			...
			*/
			...
			/*if(null!=hashTag && hashTag!="" && photoDescription.indexOf(hashTag)==-1) {
				continue;
			}*/
	...
	insertFacebookPhotos = function(response,$photomaxContainer,createFlag,loadMoreFlag,paginateFlag) {
		...
		if(photomax_global_options.loadMode.indexOf("paginate")==-1) {
			//if not pagination then update tokens..
			if(null!=nextPageUrl && null!=nextPageUrl.next) {
				nextPageUrl = nextPageUrl.next;
				if(nextPageUrl.indexOf("photomax-generated")==-1) {
					$photomaxLoadMoreDiv.data('nextpagetoken',nextPageUrl);
				}
			} else {
				$photomaxLoadMoreDiv.data('nextpagetoken','');
			}
		...
		*/
		...
		for(var i=0; i<photoArray.length; i++) {
			...
			photoHD = extractAlbumImage(photoArray[i]);
	...
	createItem = function(network,photoId,photoTitle,photoDescription,photoViewCount_raw,photoLikeCount_raw,photoCommentCount_raw,photoUploaded,photoLink,photoThumbnail,photoHD,photomax_global_options,photomax_translator_text,videoLink) {
			...
			/*if(network=="instagram") {
				primaryAttributeString = '<span class="photomax-list-thumbnail-icon"><i class="fa fa-heart"></i></span> <span class="photomax-thumbnail-primary-attribute">' + videoLikeCount+'</span> <span class="photomax-views-text">'+photomax_translator_text.likes+'</span> ';
			} else {
				primaryAttributeString = '<span class="photomax-list-thumbnail-icon"><i class="fa fa fa-dot-circle-o"></i></span> <span class="photomax-all-skin-views">' + videoViewCount+'</span> <span class="photomax-views-text">'+photomax_translator_text.views+'</span> ';
			}
			
			if (photomax_global_options.displayType=="thumbnail") {
				frame_source = generateFrameSource(videoId,network,false,photomax_global_options);				
				thumbnailString = '<div class="fluid-width-video-wrapper" style="padding-top:'+(photomax_global_options.aspectRatio*100)+'%;"><iframe id="photomax-encloser-video" style="width:100%;" src="'+frame_source+'" frameborder="0" allowfullscreen></iframe></div>';
			} else if (photomax_global_options.displayType=="link") {
				thumbnailString = '<a href="'+videoLink+'" target="_blank"><img class="photomax-main-thumbnail" href="'+videoLink+'" src="'+videoThumbnail+'"></a>';
			} else {
				thumbnailString = '<img class="photomax-main-thumbnail" href="'+photoLink+'" src="'+photoThumbnail+'">';
			}*/			
			...
			if(photomax_global_options.skin.indexOf("list")!=-1) {
				...
				//item += '<span class="photomax-view-date-holder"><span class="photomax-video-list-views" title="views"><span class="photomax-list-thumbnail-icon"><i class="fa fa fa-dot-circle-o"></i></span> <span class="photomax-all-skin-views">' + videoViewCount+'</span> <span class="photomax-views-text">'+photomax_translator_text.views+'</span></span> <span class="photomax-views-date-separator">|</span><span class="photomax-video-list-date">'+getDateDiff(videoUploaded,photomax_translator_text)+'</span> <span class="photomax-video-list-likes" title="likes"> <span class="photomax-list-thumbnail-icon"><i class="fa fa fa-heart"></i></span> <span class="photomax-all-skin-likes">'+videoLikeCount+'</span> <span class="photomax-views-text">'+photomax_translator_text.likes+'</span></span><span class="photomax-video-list-comments" title="comments"> <span class="photomax-list-thumbnail-icon"><i class="fa fa fa-comment"></i></span> <span class="photomax-all-skin-comments">'+videoCommentCount+'</span> <span class="photomax-views-text">'+photomax_translator_text.comments+'</span></span></span></p>';
				...
				//item += '<span class="photomax-view-date-holder"><span class="photomax-video-list-views" title="views">'+primaryAttributeString+'</span><span class="photomax-views-date-separator">|</span><span class="photomax-video-list-date">'+getDateDiff(photoUploaded,photomax_translator_text)+'</span></span></p>';
			...
			/*
			if(photomax_global_options.skin.indexOf("clean")!=-1) {
				if(network=="vimeo" && photomax_global_options.showVimeoLikesInsteadOfViews) {
					item += '<div class="photomax-definition"><i class="fa fa-heart fa-1x"></i> <span class="photomax-all-skin-likes">'+videoLikeCount+'</span> </div>';
					item += '<div class="photomax-duration"><i class="fa fa-comment fa-1x"></i> <span class="photomax-all-skin-comments">'+videoCommentCount+'</span> </div>';
				} else {
					item += '<div class="photomax-duration"><i class="fa fa-heart fa-1x"></i> <span class="photomax-all-skin-likes">'+videoLikeCount+'</span> </div>';
					item += '<div class="photomax-definition"><i class="fa fa-volume-off fa-1x"></i> <span class="photomax-all-skin-views">'+videoViewCount+'</span> </div>';
				}
				
				item += '<div class="photomax-clean-overlay-holder"><span class="photomax-clean-title">'+videoTitle+'</span><span class="photomax-clean-time">'+videoDuration+'</span></span></div>';
				
			} else {
			
				if (photomax_global_options.displayType!="thumbnail") {
					item += '<div class="photomax-duration">'+videoDuration+'</div>';
					
					if(network=="youtube") {
						item += '<div class="photomax-definition">'+videoDefinition+'</div>';
					}
				}
				
				if(photomax_global_options.skin.indexOf("block")!=-1) {
					item += '<div class="photomax-like-comment-holder"><div class="photomax-like-box"><i class="fa fa-heart"></i> <span class="photomax-all-skin-likes">'+videoLikeCount+'</span></div><div class="photomax-comment-box"><i class="fa fa-comment"></i><span class="photomax-all-skin-comments">'+videoCommentCount+'</span></div></div>';
				}
			}
			*/
	...
	createGrid = function($photomaxContainer,itemType,createFlag,loadMoreFlag,paginateFlag,$items) {
		...
		var options = {
		  ...
		  //container: $photomaxContainer.find('#photomax-video-list-div'), // Optional, used for some extra CSS styling
		  offset: photomax_global_options.innerOffset, // Optional, the distance between grid items
		  itemWidth: photomax_global_options.minItemWidth, // Optional, the width of a grid item
		  flexibleWidth : photomax_global_options.maxItemWidth,
		  outerOffset: photomax_global_options.outerOffset
		};

		var photomax_wookmark;*/
			
		//$photomaxContainerList.imagesLoaded(function() {
		...
		$photomaxContainerList.imagesLoaded().always(function() {
			...
			//$photomaxContainerList.css("opacity","1");
			
			//photomax_wookmark = $photomaxContainerList.wookmark(options);
			//setTimeout(function(){ photomax_wookmark.wookmarkInstance.updateOptions(); }, photomax_global_options.updateLayoutDelay);
			...
			if(loadMoreFlag) {
				...
				/*var result = */$photomaxContainerList.masonry('remove',$oldItems).masonry('layout');
				//console.log(result);
			...
			if(itemType=="album") {
				//in next version
				/*if(photomax_global_options.playlistAction=="playall") {
					registerPopup($photomaxContainer,true);
				} else {
					$photomaxContainer.find('#photomax-video-list-div .photomax-main-thumbnail').click(function(){
						...
						photomaxPlaylistId = $photomaxPlaylistThumbnail.attr("id");
						displayItems(photomaxPlaylistId,$photomaxContainer);
						photomax_current_playlist_name = $photomaxPlaylistThumbnail.find('.photomax-video-list-title').text();
	...
	registerPopup = function($photomaxContainer,isPlaylist) {
		...
		if(photomax_global_options.displayType=="popup") {
			...
			/*if(isPlaylist) {
				youtube_frame_source = photomax_global_options.photoProtocol + "//www.youtube.com/embed?listType=playlist&list=%id%&rel=0";
			} else {
				youtube_frame_source = photomax_global_options.photoProtocol + "//www.youtube.com/embed/%id%?rel=0";
			}
			if(photomax_global_options.autoPlayVideo) {
				youtube_frame_source+="&autoplay=1";
			}
			if(photomax_global_options.showTitleInVideoPlayer) {
				youtube_frame_source+="&showinfo=1";
			} else {
				youtube_frame_source+="&showinfo=0";
			}
			youtube_frame_source+="&theme="+photomax_global_options.videoPlayerTheme;

			//vimeo frame source
			vimeo_frame_source = photomax_global_options.photoProtocol + "//player.vimeo.com/video/%id%";
			if(photomax_global_options.autoPlayVideo) {
...
'</div>'+	
	...
	displayPhotoData = function($baseElement,$photomaxContainer) {
		...
		setTimeout(function(){
			...
			if($photomaxPlayBox.find('.media-popup-stat-1').width()<150) {
				$photomaxPlayBox.find('.popup-stat-text').hide();
			}
			...
			/*videoUrl = $photomaxContainer.find('.mfp-content iframe').attr('src');
	...
	displayItems = function(tabId,$photomaxContainer) {
		...
		if(tabId.indexOf("instagram_user_photos_")!=-1) {
			innerId=tabId.substring(22);
			getInstagramUserPhotos(innerId,null,$photomaxContainer);
		} else if(tabId.indexOf("instagram_user_tagged_photos_")!=-1) {
			innerId=tabId.substring(29);
			getInstagramUserTaggedPhotos(innerId,null,$photomaxContainer);
		} else if(tabId.indexOf("instagram_global_tagged_photos_")!=-1) {
			innerId=tabId.substring(31);
			getInstagramGlobalTaggedPhotos(innerId,null,$photomaxContainer);
		} else if(tabId.indexOf("google_album_photos_")!=-1) {
			innerId=tabId.substring(20);
			getGoogleAlbumPhotos(innerId,null,$photomaxContainer);
		} else if(tabId.indexOf("google_user_photos_")!=-1) {
			innerId=tabId.substring(19);
			getGoogleUserPhotos(innerId,null,$photomaxContainer);
		} else if(tabId.indexOf("facebook_album_photos_")!=-1) {
			innerId=tabId.substring(22);
			getFacebookAlbumPhotos(innerId,null,$photomaxContainer);
		} else if(tabId.indexOf("facebook_page_photos_")!=-1) {
			innerId=tabId.substring(21);
			getFacebookPagePhotos(innerId,null,$photomaxContainer);
		}	
	...
	createTabs = function($photomaxContainer) {
		...
		if(photomax_global_options.alwaysUseDropdown) {
			...
			$photomaxContainer.find('#photomax-select-box').hide();
			$photomaxContainer.find('#photomax-tabs').show();
		}*/
	...
	scrapeGoogleAlbumId = function(source_url) {
		...
		s=source_url.indexOf("/albums/");
		u=source_url.indexOf("/",s+8);
		t=source_url.indexOf("/photos/");
		v=source_url.indexOf("/",t+8);
		//console.log('s-'+s);
		if(s!=-1) {
			if(u==-1) {
				albumId = source_url.substring(s+8);
			} else {
				albumId = source_url.substring(s+8,u);
			}
			userId = source_url.substring(t+8,v);
	...
	/*initHeader = function($photomaxContainer) {
	
		var photomax_global_options = $photomaxContainer.data('photomax_global_options');
		var s;
		
		//Get Channel header and details 
		if(photomax_global_options.channel!=null) {
			s=photomax_global_options.channel.indexOf("/user/");
			//console.log('s-'+s);
			if(s!=-1) {
				userId = photomax_global_options.channel.substring(s+6);
				//console.log('userId-'+userId);
				apiUrl = "https://www.googleapis.com/youtube/v3/channels?part=id&forUsername="+userId+"&key="+photomax_global_options.apiKey;
				getChannelId(apiUrl,$photomaxContainer);
			} else {
				s=photomax_global_options.channel.indexOf("/channel/");
				if(s!=-1) {
					channelId = photomax_global_options.channel.substring(s+9);
					//console.log('channelId-'+channelId);
					getChannelDetails(channelId,$photomaxContainer);
				} else {
    ...
    $.fn.photomax = function(options) {
		...
		photomax_global_options.instagramClientId = options.instagramClientId||'adf4c4ffe0d746ab8552e5c5b0210f94';
		...
		if(photomax_global_options.hidePhotoDetails) {
			custom_styles += photomaxElementId+' .photo-popup-title,'+photomaxElementId+' .photo-popup-description,'+photomaxElementId+' .photo-popup-stats{display:none !important;}';
		}
...
/*
function photomaxSaveToken(authResult) {
	//console.log(authResult);
	if (authResult['status']['signed_in']) {
		photomaxLoggedInUser.photomaxAccessToken = authResult.access_token;
		jQuery('.photomax-add-comment-button').removeAttr('disabled').html('<i class="fa fa-send fa-2x"></i>');
